...
/**  
* This class is responsible for recording the audio
...
public class AudioRecorder {
	...
	/**
	 * The interval in which the recorded samples are output to the file Used
	...
	/**
	 * Apply gain - Supported only in uncompressed recording // Gain converted
	 * from dB to multiplier; // Based on info found in WaveGain application //
	...
	/**
	 * 
	 * Returns the state of the recorder in a AudioRecord.State typed object.
	...
	private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
		...
		public void onPeriodicNotification(AudioRecord recorder) {
			...
			try {
				if (bSamples == 16) {
					shBuffer.rewind();
					int bLength = shBuffer.capacity(); // Faster than accessing
														// buffer.capacity each
														// time
					...
					int bLength = bBuffer.capacity(); // Faster than accessing
														// buffer.capacity each
														// time
					bBuffer.rewind();
	...
	public AudioRecorder(boolean uncompressed, int audioSource, int sampleRate,
		...
		try {
			...
			if (rUncompressed) { // RECORDING_UNCOMPRESSED
				...
				aRecorder.setRecordPositionUpdateListener(updateListener,new Handler());
			...
			if (state == State.INITIALIZING) {
				if (rUncompressed) {
					if ((aRecorder.getState() == AudioRecord.STATE_INITIALIZED)
						...
						bBuffer = ByteBuffer.allocateDirect(framePeriod
								* bSamples / 8 * nChannels);
	...
	public void start() {
		if (state == State.READY) {
			if (rUncompressed) {
				...
				//aRecorder.read(new byte[1024], 0, 1024);
				
				//aRecorder.set
				//byte data[] = new byte[minBuf];
				//aRecorder.read(data,0, minBuf);
	...
	public void stop() {
		if (state == State.RECORDING) {
			...
			broadcastIntent.putExtra("url", fPath);
			broadcastIntent
					.setAction("com.carenx.ACTION_RECORDING_COMPLETE");
			RecorderActivity.mContext.sendBroadcast(broadcastIntent);
